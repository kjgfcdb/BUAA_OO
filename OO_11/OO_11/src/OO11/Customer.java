...
class Customer {
    ...
    Customer(int start, int end, long time, MyPrintWriter out) {
        /*
         * @REQUIRES:0<=start<=6399 && 0<=end<=6399 && time>=0 && out!=null
         * @MODIFIES:
         *      \this.start;
         *      \this.end;
         *      \this.time;
    ...
    public boolean repOK() {
         ...
         *      \result == (0<=start<6400) &&
         *      (0<=end<6400) && (time>=0) &&
         *      (out!=null) && (buffer!=null) && (taxis!=null);
         */
        return (start>=0 && start<6400) &&
                (end>=0 && end<6400) &&
                (time>=0) && (out!=null) &&
                (buffer!=null) && (taxis!=null);
    }
    ...
    void setId(int id) {
         ...
         * @EFFECTS:
         *      \this.id==id;
         *      \this.buffer.append("Customer#id (start/TaxiSys.SIZE+1,start%TaxiSys.SIZE+1) ->
          *      (end/TaxiSys.SIZE+1,end%TaxiSys.SIZE+1) time first Logging :\n");
         */
        this.id = id;
        String s = "Customer#"+id+" ("+(start/TaxiSys.SIZE+1)+","+(start%TaxiSys.SIZE+1)+") "
                +"->"+" ("+(end/TaxiSys.SIZE+1)+","+(end%TaxiSys.SIZE+1)+") "+time+" first Logging :\n";
        buffer.append(s);
    }
    ...
    boolean tryToGetOn() {//是否能上车
        ...
        String head = "Taxis tried to grab "+this+"'s order: \n";
        buffer.append(head);
        int format = 0;
        for (Taxi t : taxis) {
            buffer.append(t);

...
class ALSElevator implements movingMachine{
    /**
     * Overview：电梯类，用于描述电梯的当前位置、状态、时间等信息。
     * AF(c) = {curPos,sta,curTime,comTime,lastReq} where c.curPos = curPos,
     *      c.sta = sta,c.curTime = curTime,c.comTime = comTime,c.lastReq =
     *      lastReq;
     */
    public int curPos;//当前位置
    public int sta;//当前状态，1表示UP,-1表示DOWN,0表示STILL
    public double curTime;//当前时间
    public double comTime;
    public String lastReq;//上一条请求

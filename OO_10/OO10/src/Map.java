...
class TrafficLight implements Runnable{
    ...
    TrafficLight(String filename) {
         ...
         */
        this.period = new Random().nextInt(51)+50;//满足[50,100]=[0,50]+50
        this.cycle = 1;
    ...
    private boolean init(String filename) {
        ...
        try {
            ...
            while ((line = bufferReader.readLine()) != null) {
                ...
                if (_line.length() != TaxiSys.SIZE) return false;//长度不对报错
                String[] strArray = _line.split("");
                for (int i = 0; i < TaxiSys.SIZE; i++) {//非法字符报错
                    if (_line.charAt(i) != '0' && _line.charAt(i) != '1')
                        return false;
                    this.tfLight[lineCnt*TaxiSys.SIZE+i] = Integer.parseInt(strArray[i]);
                }
                lineCnt++;
            }
            if (lineCnt != TaxiSys.SIZE) return false;//行数不对报错
